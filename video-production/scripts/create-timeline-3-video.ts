...
class Timeline3VideoCreator {
  ...
  private async recordVideoSegments() {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      // Record video with interactions for this segment
  ...
  private async performSegmentInteractions(segment: TimelineSegment) {
    ...
    switch (segment.name) {
      ...
      case 'technical_architecture':
        ...
        try {
          const mapControls = await this.page.locator('.mapboxgl-ctrl-group').all();
      ...
      case 'simplified_flow':
        ...
        try {
          // Try to zoom in/out
          const zoomControls = await this.page.locator('.mapboxgl-ctrl-zoom-in, .mapboxgl-ctrl-zoom-out').all();
      ...
      case 'conclusion':
        ...
        try {
          const summaryElements = await this.page.locator('.dashboard-header, .zones-overview').all();

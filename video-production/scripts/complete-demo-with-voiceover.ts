...
class CompleteDemoWithVoiceover {
  ...
  async runCompletePipeline() {
    console.log('🎬 Complete Demo Video Pipeline with Voice-over');
    console.log('===============================================');
    console.log('This will create a 5:40 video with synchronized voice-over');
    console.log('');

    try {
      await this.generateVoiceOver();
      await this.recordDemoActions();
      await this.synchronizeAndAssemble();
      
      console.log('✅ Complete pipeline finished successfully!');
      console.log('📹 Final video with voice-over ready in output/ directory');
  ...
  private async generateVoiceOver() {
    console.log('🎤 Step 1: Generating voice-over narration...');
    ...
    const narrationYaml = {
      title: "Disaster Response Demo Narration",
    ...
    const narrationPath = path.join(this.outputDir, 'demo-narration.yaml');
    fs.writeFileSync(narrationPath, JSON.stringify(narrationYaml, null, 2));

    // Create timeline YAML
    const timelineYaml = {
      title: "Disaster Response Demo Timeline",
      total_duration: 340,
      scenes: scenes.map(scene => ({
        id: scene.id,
        start_time: scene.startTime,
        end_time: scene.startTime + scene.duration,
        duration: scene.duration
      }))
    };

    const timelinePath = path.join(this.outputDir, 'demo-timeline.yaml');
    fs.writeFileSync(timelinePath, JSON.stringify(timelineYaml, null, 2));

    console.log('📝 Created narration and timeline YAML files');

    // Generate voice-over using TTS script
    try {
      console.log('🎤 Generating voice-over audio...');
      const ttsCommand = `cd "${__dirname}/.." && source venv/bin/activate && python3 scripts/tts_providers.py "${narrationPath}" "${timelinePath}"`;
      execSync(ttsCommand, { stdio: 'inherit' });
      console.log('✅ Voice-over generation completed');
    } catch (error) {
      console.error('❌ Voice-over generation failed:', error);
      console.log('⚠️  Continuing without voice-over...');
    }
  }

  private async recordDemoActions() {
    console.log('📹 Step 2: Recording demo actions...');
  ...
  private async synchronizeAndAssemble() {
    console.log('🎬 Step 3: Synchronizing video and audio...');
    ...
    try {
      ...
      if (audioFiles.length > 0) {
        console.log(`🎤 Found ${audioFiles.length} audio files, combining with video...`);
        
        // Create a combined audio file
        const combinedAudioPath = path.join(this.outputDir, 'combined-narration.wav');
        const audioFilePaths = audioFiles.map(file => path.join(this.audioDir, file));
        
        // Combine all audio files
        const audioListPath = path.join(this.outputDir, 'audio-list.txt');
        const audioListContent = audioFilePaths.map(path => `file '${path}'`).join('\n');
        fs.writeFileSync(audioListPath, audioListContent);
        
        const combineAudioCommand = `ffmpeg -f concat -safe 0 -i "${audioListPath}" -c copy "${combinedAudioPath}"`;
        execSync(combineAudioCommand, { stdio: 'inherit' });
        
        // Combine video with audio
        const finalOutputPath = path.join(this.outputDir, 'complete-demo-with-voiceover.mp4');
        const combineCommand = `ffmpeg -i "${videoFile.path}" -i "${combinedAudioPath}" -c:v copy -c:a aac -shortest "${finalOutputPath}"`;
        execSync(combineCommand, { stdio: 'inherit' });
        
        console.log(`✅ Final video with voice-over generated: ${finalOutputPath}`);
        ...
        console.log('⚠️  No audio files found, creating video-only version...');
        
        // Convert webm to mp4 without audio
        const outputPath = path.join(this.outputDir, 'complete-demo-video-only.mp4');
        const ffmpegCommand = `ffmpeg -i "${videoFile.path}" -c:v libx264 -preset fast -crf 23 "${outputPath}"`;
        execSync(ffmpegCommand, { stdio: 'inherit' });
        
        console.log(`✅ Video-only version generated: ${outputPath}`);
      }
      
    } catch (error) {
      console.error('❌ Failed to synchronize video and audio:', error);
      throw error;
    }
  }
...
// Run the complete pipeline with voice-over
const pipeline = new CompleteDemoWithVoiceover();

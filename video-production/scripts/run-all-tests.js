...
class MasterTestRunner {
  constructor() {
    this.testResults = [];
    this.testScripts = [
      'test-enhanced-captures.test.js',
      'test-production-pipeline.test.js',
  ...
  async runAllTests() {
    ...
    const captureTestResult = await this.runTest('test-enhanced-captures.test.js');
    
    console.log('\n' + '='.repeat(80));
    console.log('🧪 TESTING PRODUCTION PIPELINE');
    console.log('='.repeat(80));
    
    const pipelineTestResult = await this.runTest('test-production-pipeline.test.js');
    
    console.log('\n' + '='.repeat(80));
    console.log('🧪 TESTING CONFIGURATION FILES');
    console.log('='.repeat(80));
    
    const configTestResult = await this.runTest('test-configuration.test.js');

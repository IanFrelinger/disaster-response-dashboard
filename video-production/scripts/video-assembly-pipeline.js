...
class VideoAssemblyPipeline {
    ...
    async assembleVideo() {
        console.log('🎬 Assembling video from timeline...');
        ...
        const outputPath = path.join(this.outputDir, 'disaster-response-complete.mp4');
        ...
        for (const track of timeline.timeline.tracks.video) {
            ...
            // Add to filter complex for timing, scaling, and transitions
            filterComplex.push(`[${inputIndex}:v]trim=duration=${track.duration},setpts=PTS-STARTPTS,scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2[v${inputIndex}]`);
            inputIndex++;
        }
    ...
    async run() {
        console.log('🎬 Disaster Response Dashboard - Video Assembly Pipeline');
        console.log('=====================================================\n');
        ...
        try {
            ...
            // Step 4: Validate final video
            const finalValidation = await this.validateFinalVideo(outputPath);
            
            if (finalValidation) {
                console.log('\n🎉 Video production pipeline completed successfully!');
            ...
            console.error('\n❌ Video production pipeline failed:', error.message);
            throw error;
        }
    }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
    const pipeline = new VideoAssemblyPipeline();
...
export default VideoAssemblyPipeline;

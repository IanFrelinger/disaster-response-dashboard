...
interface Interaction {
  type: 'click' | 'hover' | 'type' | 'drag' | 'select' | 'wait' | 'screenshot' | 'waitForSelector';
  selector?: string;
  value?: string;
  duration?: number;
  description: string;
...
class InteractivePresentationRecorder {
  ...
  constructor() {
    this.config = this.createInteractiveConfig();
    this.ensureOutputDirs();
  }

  private createInteractiveConfig(): InteractiveConfig {
    return {
      app: {
      ...
      beats: [
        {
          ...
          interactions: [
            ...
            { type: 'wait', duration: 2000, description: 'Let page settle' },
        ...
        },
        {
          ...
          interactions: [
            ...
            { type: 'waitForSelector', selector: '.mapboxgl-canvas', description: 'Wait for map to load' },
            { type: 'wait', duration: 2000, description: 'Let map render' },
            { type: 'hover', selector: '.hazard-layer-toggle', description: 'Hover over hazard layer toggle' },
            { type: 'click', selector: '.hazard-layer-toggle', description: 'Enable hazard layers' },
            { type: 'wait', duration: 1000, description: 'Wait for hazards to appear' },
        ...
        },
        {
          ...
          interactions: [
            ...
            { type: 'wait', duration: 1000, description: 'Wait for dashboard to load' },
            { type: 'hover', selector: '.zone-card', description: 'Hover over zone summary card' },
            { type: 'click', selector: '.zone-card', description: 'Expand zone details' },
            { type: 'wait', duration: 1000, description: 'Wait for details to expand' },
            ...
            { type: 'wait', duration: 1000, description: 'Wait for map to load' },
        ...
        },
        {
          ...
          interactions: [
            ...
            { type: 'hover', selector: '.api-status-indicator', description: 'Hover over API status' },
            { type: 'click', selector: '.api-status-indicator', description: 'Show API details' },
            { type: 'wait', duration: 1000, description: 'Wait for API panel' },
        ...
        },
        {
          ...
          interactions: [
            ...
            { type: 'waitForSelector', selector: '.mapboxgl-canvas', description: 'Wait for map to load' },
            { type: 'wait', duration: 2000, description: 'Let map render' },
            { type: 'hover', selector: '.hazard-marker', description: 'Hover over hazard marker' },
            { type: 'click', selector: '.hazard-marker', description: 'Select hazard for details' },
            { type: 'wait', duration: 1000, description: 'Wait for hazard details' },
        ...
        },
        {
          ...
          interactions: [
            { type: 'click', selector: 'button:has-text("Commander Dashboard")', description: 'Switch to dashboard' },
            { type: 'wait', duration: 2000, description: 'Wait for dashboard to load' },
            { type: 'hover', selector: '.risk-assessment-panel', description: 'Hover over risk assessment' },
            { type: 'click', selector: '.risk-assessment-panel', description: 'Expand risk details' },
            { type: 'wait', duration: 1000, description: 'Wait for risk details' },
        ...
        },
        {
          ...
          interactions: [
            ...
            { type: 'waitForSelector', selector: '.mapboxgl-canvas', description: 'Wait for map to load' },
            { type: 'wait', duration: 2000, description: 'Let map render' },
            { type: 'click', selector: '.drawing-tool', description: 'Activate drawing tool' },
            { type: 'wait', duration: 1000, description: 'Wait for drawing mode' },
            { type: 'drag', selector: '.mapboxgl-canvas', description: 'Draw evacuation zone' },
            { type: 'wait', duration: 2000, description: 'Wait for zone to be created' },
            { type: 'screenshot', description: 'Capture zone definition' }
          ],
        ...
        },
        {
          ...
          interactions: [
            { type: 'click', selector: '.route-planning-panel', description: 'Open route planning' },
            { type: 'wait', duration: 1000, description: 'Wait for route panel' },
            { type: 'select', selector: '.route-profile-selector', value: 'civilian', description: 'Select civilian route profile' },
            { type: 'wait', duration: 1000, description: 'Wait for routes to calculate' },
            { type: 'hover', selector: '.route-line', description: 'Hover over route' },
            { type: 'click', selector: '.route-line', description: 'Select route for details' },
            { type: 'wait', duration: 1000, description: 'Wait for route details' },
        ...
        },
        {
          ...
          interactions: [
            { type: 'click', selector: 'button:has-text("Commander Dashboard")', description: 'Switch to dashboard' },
            { type: 'wait', duration: 2000, description: 'Wait for dashboard to load' },
            { type: 'hover', selector: '.unit-card', description: 'Hover over unit card' },
            { type: 'drag', selector: '.unit-card', description: 'Drag unit to map' },
            { type: 'wait', duration: 1000, description: 'Wait for unit assignment' },
            { type: 'hover', selector: '.building-status-card', description: 'Hover over building status' },
            { type: 'click', selector: '.building-status-card', description: 'Expand building details' },
            { type: 'wait', duration: 1000, description: 'Wait for building details' },
        ...
        },
        {
          ...
          interactions: [
            ...
            { type: 'waitForSelector', selector: '.mapboxgl-canvas', description: 'Wait for map to load' },
            { type: 'wait', duration: 2000, description: 'Let map render' },
            { type: 'click', selector: '.ai-assistant-button', description: 'Open AI assistant' },
            { type: 'wait', duration: 1000, description: 'Wait for AI panel' },
            { type: 'type', selector: '.ai-input-field', value: 'What happens if we lose Highway 30?', description: 'Type AI query' },
            { type: 'click', selector: '.ai-send-button', description: 'Send AI query' },
            { type: 'wait', duration: 3000, description: 'Wait for AI response' },
            { type: 'screenshot', description: 'Capture AI assistant interaction' }
          ],
        ...
        },
        {
          ...
          interactions: [
            { type: 'click', selector: 'button:has-text("Commander Dashboard")', description: 'Switch to dashboard' },
            { type: 'wait', duration: 2000, description: 'Wait for dashboard to load' },
            { type: 'hover', selector: '.metrics-panel', description: 'Hover over metrics panel' },
            { type: 'click', selector: '.metrics-panel', description: 'Expand metrics details' },
            { type: 'wait', duration: 1000, description: 'Wait for metrics to load' },
        ...
        },
        {
          ...
          interactions: [
            ...
            { type: 'waitForSelector', selector: '.mapboxgl-canvas', description: 'Wait for map to load' },
            { type: 'wait', duration: 2000, description: 'Let map render' },
            { type: 'click', selector: '.foundry-integration-panel', description: 'Open Foundry integration panel' },
            { type: 'wait', duration: 1000, description: 'Wait for Foundry panel' },
            { type: 'hover', selector: '.data-pipeline-indicator', description: 'Hover over data pipeline' },
            { type: 'click', selector: '.data-pipeline-indicator', description: 'Show pipeline details' },
            { type: 'wait', duration: 1000, description: 'Wait for pipeline details' },
        ...
        },
        {
          ...
          interactions: [
            { type: 'click', selector: 'button:has-text("Commander Dashboard")', description: 'Switch to dashboard' },
            { type: 'wait', duration: 2000, description: 'Wait for dashboard to load' },
            { type: 'hover', selector: '.nav-menu-toggle', description: 'Hover over navigation menu' },
            { type: 'click', selector: '.nav-menu-toggle', description: 'Open navigation menu' },
            { type: 'wait', duration: 1000, description: 'Wait for menu to open' },
            { type: 'hover', selector: '.cta-button', description: 'Hover over call to action button' },
            { type: 'click', selector: '.cta-button', description: 'Click call to action' },
            { type: 'wait', duration: 1000, description: 'Wait for CTA response' },
  ...
  async initialize(): Promise<boolean> {
    console.log('🚀 Initializing Playwright browser for interactive presentation...');
  ...
  private async executeInteraction(interaction: Interaction): Promise<boolean> {
    if (!this.page) {
    ...
    try {
      ...
      switch (interaction.type) {
        ...
        case 'drag':
          if (interaction.selector) {
            // Simple drag simulation - click and move mouse
            await this.page.click(interaction.selector);
        ...
        case 'screenshot':
          ...
          await this.page.screenshot({ 
            path: path.join(this.outputDir, `interactive-${timestamp}.png`),
            fullPage: true 
          });
          break;
      }
  ...
  async generateTTSAudio(beat: InteractiveBeat): Promise<string | null> {
  ...
  async recordBeat(beat: InteractiveBeat): Promise<BeatResult> {
    console.log(`\n🎬 Recording interactive beat: ${beat.title} (${beat.duration}s)`);
    ...
    try {
      ...
      console.log(`✅ Interactive beat completed successfully (${result.actualDuration.toFixed(1)}s)`);
      
    } catch (error) {
      result.error = `Interactive beat recording failed: ${error}`;
      console.error(`❌ Interactive beat failed: ${error}`);
    }
  ...
  async saveRecordedVideo(): Promise<void> {
    ...
    try {
      ...
      const targetPath = path.join(this.outputDir, 'disaster-response-interactive.mp4');
      ...
      try {
        ...
        console.log(`✅ Interactive video saved as: ${targetPath}`);
        ...
        const newPath = path.join(this.outputDir, 'disaster-response-interactive.webm');
        fs.renameSync(sourcePath, newPath);
        console.log(`✅ Interactive video saved as: ${newPath}`);
      }
      
    } catch (error) {
      console.error('❌ Failed to save interactive video:', error);
    }
  }

  async generateReport(): Promise<void> {
    const reportPath = path.join(this.outputDir, 'interactive-presentation-report.json');
    ...
    console.log('\n🎬 Next Steps:');
    console.log('1. Review the recorded interactive video in the captures directory');
  ...
  async runInteractivePresentation(): Promise<void> {
    console.log('🎬 Starting Interactive Presentation Recording');
    ...
    try {
      ...
      console.error('❌ Interactive presentation failed:', error);
    } finally {
      if (this.page) {
...
// Main execution
async function main() {
  const recorder = new InteractivePresentationRecorder();
  await recorder.runInteractivePresentation();
}
...
export { InteractivePresentationRecorder };

...
class CompleteDemoPipeline {
  ...
  constructor() {
    ...
    [this.outputDir, this.audioDir, this.capturesDir].forEach(dir => {
  ...
  async runCompletePipeline() {
    console.log('🎬 Complete Demo Video Pipeline');
    console.log('================================');
    console.log('This will:');
    console.log('1. Record screen actions with precise timing');
    console.log('2. Generate voice-over narration');
    console.log('3. Synchronize audio and video');
    console.log('4. Assemble final presentation');
    console.log('');

    try {
      // Step 1: Record the demo actions
      await this.recordDemoActions();
      
      // Step 2: Generate voice-over
      await this.generateVoiceOver();
      
      // Step 3: Synchronize and assemble
  ...
  private async executeDemoTimeline() {
    ...
    console.log(`📹 Executing ${scenes.length} scenes over 5:40...`);
    
    for (const scene of scenes) {
      console.log(`🎬 Scene: ${scene.title} (${scene.startTime}s - ${scene.startTime + scene.duration}s)`);
      
      // Wait for scene start time
      if (scene.startTime > 0) {
        await this.page!.waitForTimeout(scene.startTime * 1000);
      }
  ...
  private async generateVoiceOver() {
    console.log('🎤 Step 2: Generating voice-over narration...');
    
    // Create narration YAML file
    const narrationData = {
      metadata: {
    ...
    const narrationPath = path.join(__dirname, '..', 'demo-narration.yaml');
    fs.writeFileSync(narrationPath, yaml.dump(narrationData));
    
    // Generate TTS using existing infrastructure
    try {
      execSync(`cd "${__dirname}/.." && python3 scripts/tts_providers.py demo-narration.yaml --provider elevenlabs`, { 
        stdio: 'inherit' 
      });
  ...
  private async synchronizeAndAssemble() {
    console.log('🎬 Step 3: Synchronizing and assembling final video...');
    ...
    try {
      ...
      const videoPath = path.join(this.capturesDir, videoFile);
      const outputPath = path.join(this.outputDir, 'complete-demo-final.mp4');
      ...
      const audioFilesContent = audioFiles.map(file => `file '${path.join(this.audioDir, file)}'`).join('\n');
      fs.writeFileSync(audioListPath, audioFilesContent);
      
      const combinedAudioPath = path.join(this.outputDir, 'combined-audio.wav');
      
      // Combine audio files
      execSync(`ffmpeg -f concat -safe 0 -i "${audioListPath}" -c copy "${combinedAudioPath}"`, { 
        stdio: 'inherit' 
      });
      
      // Combine video and audio
      execSync(`ffmpeg -i "${videoPath}" -i "${combinedAudioPath}" -c:v copy -c:a aac -shortest "${outputPath}"`, { 
        stdio: 'inherit' 
      });
...
const pipeline = new CompleteDemoPipeline();

...
class ProductionPipelineTester {
  ...
  testScriptContent() {
    ...
    try {
      ...
      contentTests++;
      if (content.includes('class EnhancedProductionPipeline')) {
        this.log('EnhancedProductionPipeline class found', 'success');
        contentPasses++;
      } else {
        this.log('EnhancedProductionPipeline class missing', 'error');
      }

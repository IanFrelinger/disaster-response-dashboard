  ...
  humanizeActions(originalActions: string[]): HumanizedAction[] {
    ...
    for (let i = 0; i < originalActions.length; i++) {
      ...
      if (nextAction && this.actionHasCoordinates(nextAction)) {
        ...
        if (nextCoords && parsedAction.coordinates) {
          const path = this.generateHumanizedPath(parsedAction.coordinates, nextCoords);
          humanizedActions.push({
            action: 'mouseMove',
            path: path,
...
// Test the InteractionBot
async function testInteractionBot() {
  console.log('🤖 Testing InteractionBot...\n');
  
  const interactionBot = new InteractionBot();
  
  // Sample actions from the record.config.json
  const sampleActions = [
    "click(text=Commander Dashboard)",
    "wait(500)",
    "mouseMove(300,420)",
    "overlay(lowerThird:Incident Commander · Planner · Dispatcher,in,0)",
    "wait(2000)",
    "overlay(lowerThird,out,300)"
  ];
  
  console.log('📝 Original Actions:');
  sampleActions.forEach((action, index) => {
    console.log(`  ${index + 1}. ${action}`);
  });
  
  console.log('\n🤖 Humanized Actions:');
  const humanizedActions = interactionBot.humanizeActions(sampleActions);
  ...
  humanizedActions.forEach((action, index) => {
    const details = [
      ...
      action.path ? `path: ${action.path.length} points` : '',
      action.duration ? `duration: ${action.duration}ms` : '',
      action.milliseconds ? `wait: ${action.milliseconds}ms` : '',
  ...
  console.log(`\n📊 Summary:`);
  console.log(`   Original actions: ${sampleActions.length}`);
  console.log(`   Humanized actions: ${humanizedActions.length}`);
  console.log(`   Enhancement ratio: ${(humanizedActions.length / sampleActions.length).toFixed(2)}x`);
  
  // Save the humanized actions to a file for inspection
  const outputPath = path.join(__dirname, '..', 'output', 'humanized-actions.json');
  ...
  const output = {
    originalActions: sampleActions,
    humanizedActions: humanizedActions,
    timestamp: new Date().toISOString(),
    summary: {
      originalCount: sampleActions.length,
      humanizedCount: humanizedActions.length,
      enhancementRatio: humanizedActions.length / sampleActions.length
    }
  };
  
  fs.writeFileSync(outputPath, JSON.stringify(output, null, 2));
  console.log(`\n💾 Humanized actions saved to: ${outputPath}`);
}

// Run the test
testInteractionBot().catch(console.error);

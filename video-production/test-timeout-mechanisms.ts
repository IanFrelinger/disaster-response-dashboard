...
class TimeoutMechanismTester {
  ...
  private async testEnhancedFrontendCapturesTimeout(): Promise<void> {
    ...
    try {
      ...
      // Check if captures timed out as expected (this is the expected behavior)
      if (result.includes('timed out after 30000ms')) {
        this.addResult({
          test: 'Frontend Captures Timeout',
          expectedBehavior: 'Should timeout after 30 seconds per step',
          actualBehavior: `Timed out correctly after ${duration}ms`,
          success: true,
          duration
        });
        this.log('✅ Frontend captures timeout test passed', 'success');
      } else if (result.includes('Enhanced capture generation completed successfully')) {
        this.addResult({
          test: 'Frontend Captures Timeout',
          expectedBehavior: 'Should timeout after 30 seconds per step',
          actualBehavior: 'Completed successfully (unexpected)',
          success: false,
          duration,
          details: 'Expected timeout but operation completed'
        });
        this.log('❌ Frontend captures timeout test failed', 'error');
      } else {
        this.addResult({
          test: 'Frontend Captures Timeout',
          expectedBehavior: 'Should timeout after 30 seconds per step',

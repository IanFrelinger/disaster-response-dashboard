...
# 🧪 Simplified Disaster Response Dashboard Smoke Test
# Comprehensive testing of all components

echo "🧪 Starting Smoke Test for Simplified Disaster Response Dashboard"
echo "================================================================"
...
# Test alerts
run_test "Alerts Validation" "curl -s http://localhost:5001/api/disaster-data | jq -r '.data.alerts | length'" "2"

echo -e "\n${YELLOW}2. Frontend Tests${NC}"
echo "=================="

# Test frontend accessibility
test_endpoint "Frontend Homepage" "http://localhost:3000" "Disaster Response Dashboard"

# Test public view
test_endpoint "Public View" "http://localhost:3000/public" "Emergency Response Status"

# Test field view
test_endpoint "Field View" "http://localhost:3000/field" "Field Operations"

# Test command view
test_endpoint "Command View" "http://localhost:3000/command" "Command Center"
...
run_test "API Response Time < 1s" "timeout 1 curl -s http://localhost:5001/api/disaster-data > /dev/null && echo 'fast'" "fast"

# Test frontend load time
run_test "Frontend Load Time < 2s" "timeout 2 curl -s http://localhost:3000 > /dev/null && echo 'fast'" "fast"
...
run_test "Invalid Endpoint Handling" "curl -s http://localhost:5001/api/invalid | grep -q 'Not Found'" ""

# Test invalid JSON
run_test "Invalid JSON Handling" "curl -s -X POST http://localhost:5001/api/update-resource-status -H 'Content-Type: application/json' -d '{invalid}' | grep -q 'error'" ""

echo -e "\n${YELLOW}8. Component Integration Tests${NC}"
echo "================================="

# Test that all views are accessible
run_test "All Views Accessible" "curl -s http://localhost:3000/public | grep -q 'Emergency Response Status' && curl -s http://localhost:3000/field | grep -q 'Field Operations' && curl -s http://localhost:3000/command | grep -q 'Command Center'" ""

# Test navigation between views
run_test "Navigation Links Present" "curl -s http://localhost:3000 | grep -q 'Public View' && curl -s http://localhost:3000 | grep -q 'Field View' && curl -s http://localhost:3000 | grep -q 'Command View'" ""

echo -e "\n${YELLOW}9. Design System Tests${NC}"
echo "========================="

# Test Apple design system classes
run_test "Apple Design Classes" "curl -s http://localhost:3000 | grep -q 'apple-design'" ""

# Test responsive design
run_test "Responsive Design" "curl -s http://localhost:3000 | grep -q 'container'" ""

echo -e "\n${YELLOW}10. Final Validation${NC}"
echo "======================="
...
run_test "Frontend-Backend Integration" "curl -s http://localhost:3000 | grep -q 'Disaster Response Dashboard'" ""

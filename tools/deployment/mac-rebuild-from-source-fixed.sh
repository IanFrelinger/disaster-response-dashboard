...
# Disaster Response Dashboard - Mac Rebuild from Source (Fixed Version)
# Portable script for rebuilding on any Mac environment
# This script completely removes everything and rebuilds from scratch
# Fixed Docker command syntax and improved error handling

set -e

# Script configuration
SCRIPT_NAME="mac-rebuild-from-source-fixed.sh"
...
NETWORK_NAME="disaster-mac-fixed-network"
...
cleanup() {
    ...
    if docker info >/dev/null 2>&1; then
        docker stop disaster-mac-fixed-backend disaster-mac-fixed-frontend disaster-mac-fixed-tileserver 2>/dev/null || true
        docker rm disaster-mac-fixed-backend disaster-mac-fixed-frontend disaster-mac-fixed-tileserver 2>/dev/null || true
...
main() {
    echo "🔥 Disaster Response Dashboard - Mac Rebuild from Source (Fixed)"
    echo "================================================================="
    ...
    local containers_to_stop=$(docker ps -q --filter "name=disaster-response" 2>/dev/null || true)
    if [ ! -z "$containers_to_stop" ]; then
        print_status "Found containers to stop: $containers_to_stop"
        if safe_docker "stop $containers_to_stop" "Stop containers"; then
    ...
    local containers_to_remove=$(docker ps -aq --filter "name=disaster-response" 2>/dev/null || true)
    if [ ! -z "$containers_to_remove" ]; then
        print_status "Found containers to remove: $containers_to_remove"
        if safe_docker "rm $containers_to_remove" "Remove containers"; then
            print_success "Existing containers removed"
        fi
    else
        print_success "No existing containers to remove"
    fi

    # Remove all existing images
    print_step "Removing all existing disaster-response images..."
    local images_to_remove=$(docker images --format "{{.ID}}" --filter "reference=disaster-response" 2>/dev/null || true)
    if [ ! -z "$images_to_remove" ]; then
        print_status "Found images to remove: $images_to_remove"
        if safe_docker "rmi -f $images_to_remove" "Remove images"; then
            print_success "Existing images removed"
        fi
    else
        print_success "No existing images to remove"
    fi

    # Remove all existing networks
    print_step "Removing existing networks..."
    local networks_to_remove="disaster-demo-network disaster-custom-network disaster-rebuild-network disaster-mac-robust-network disaster-mac-fixed-network"
    ...
    print_success "Existing networks cleaned"
    ...
    if safe_docker "run -d --name disaster-mac-fixed-backend --network $NETWORK_NAME -p $BACKEND_PORT:8000 -e ENVIRONMENT_MODE=demo -e USE_SYNTHETIC_DATA=true $BACKEND_TAG:latest" "Start backend"; then
    ...
    if safe_docker "run -d --name disaster-mac-fixed-tileserver --network $NETWORK_NAME -p $TILE_SERVER_PORT:8080 -v $(pwd)/tiles:/data $TILESERVER_TAG:latest" "Start tile server"; then
    ...
    echo "  Backend: disaster-mac-fixed-backend"
    echo "  Frontend: disaster-mac-fixed-frontend"
    echo "  Tile Server: disaster-mac-fixed-tileserver"
    echo "  Network: $NETWORK_NAME"
    echo ""
    echo "📋 Management Commands:"
    echo "  View logs: docker logs disaster-mac-fixed-*"
    echo "  Stop services: docker stop disaster-mac-fixed-*"
    echo "  Remove containers: docker rm disaster-mac-fixed-*"
    echo "  View status: docker ps --filter name=disaster-mac-fixed"
    echo ""
    echo "🧹 Cleanup Commands:"
    echo "  Stop and remove: docker stop disaster-mac-fixed-* && docker rm disaster-mac-fixed-*"
    ...
    docker ps --filter name=disaster-mac-fixed --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
